...
impl Kernel for CubicSpine {
    fn apply_on_norm(&self, r_norm: f32) -> f32 {
        self.unormalized_apply_on_norm(r_norm / self.h, self.normalizer)
    }
    ...
    fn gradient(&self, r: &Vector3<f32>) -> Vector3<f32> {
        ...
        6. * self.sigma * self.normalizer * r * (1. / (r_norm * self.h)) * match q {
    ...
    fn unormalized_apply_on_norm(&self, q: f32, normalizer: f32) -> f32 {
